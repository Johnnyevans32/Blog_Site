	...
	if (isset($_POST['submit'])) {
		...
		if ($query_run = mysqli_query($con, $query)) {
			...
			if ($query_num_rows== 0) {
				...
				echo "<script>window.open ('home.php?logged_in=YOU ARE LOGGED INTO ADMIN AREA!!<br>HAVE FUN BUT NOT TOO MUCH..','_self')</script>";;
			}
		}	
	}
?>
...
<body style=" min-height: 100vh; background-image: linear-gradient(120deg, #3498db, #8e44ad); ">
	<form method="POST" action="login.php" style=" width: 360px; background: #f1f1f1; height: 480px; padding: 80px 40px; border-radius: 10px; position: absolute;left: 50%;top: 70%; transform: translate(-50%, -50%); ">
